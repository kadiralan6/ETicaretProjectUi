...
const toIsoOrEmpty = (local: string) => {
  if (!local) return "";
  return new Date(local).toISOString();
};

export const AdminCampaignCreatePage = () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (payload: ICreateCampaignRequest) => {
      const res = await nextApiClient.post(
        NEXT_API_URLS.CAMPAIGNS_CREATE,
        payload,
      );
      return res.data;
    },
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    const payload: ICreateCampaignRequest = {
      name: formData.name,
      type: formData.type,
      discountValue: Number(formData.discountValue),
      minimumOrderAmount: Number(formData.minimumOrderAmount),
      startDate: toIsoOrEmpty(formData.startDate),
      endDate: toIsoOrEmpty(formData.endDate),
